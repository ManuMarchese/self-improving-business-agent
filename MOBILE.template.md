...
## 1. Servidor persistente (sin ventanitas)

El serve corre como **tarea programada de Windows** `<TU-TAREA-SERVE>`: arranca al iniciar sesión,
corre oculto (cero ventanas), y **se revive solo cada 5 minutos** si muere (probado matando el
proceso a mano). Desde <FECHA> escucha SOLO en la IP Tailscale (`<TU-IP>:4096`):
...
Start-ScheduledTask -TaskName "<TU-TAREA-SERVE>"   # arrancar ya (si está caído, no espera los 5 min)
Stop-ScheduledTask -TaskName "<TU-TAREA-SERVE>"    # detener (más: matar el proceso opencode serve)
Get-ScheduledTask -TaskName "<TU-TAREA-SERVE>"     # ver estado
...
- El <FECHA> se jubiló `opencode-serve.vbs` del Startup (ataba el puerto con la clave VIEJA y
  bloqueaba a la tarea; backup en `~/.<TU-BACKUP>/`). También se borró la clave vieja de entorno.
...
- `tailscale serve` (URL MagicDNS) NO está habilitado en este tailnet (pide admin): usar IPs directas.
  el comando `tailscale serve --bg` además SE CUELGA en esta PC (ver serve-mobile.log);
