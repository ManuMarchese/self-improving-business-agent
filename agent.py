...
_MOBILE_REPLACEMENTS = [
    ...
    ("Ma" + "nu", "<TU-NOMBRE>"),
    ("SIAFB-Mobile-Serve", "<TU-TAREA-SERVE>"),
    ("~/.siafb-backup/", "~/.<TU-BACKUP>/"),
    ("(sin ventanitas, desde 2026-09-19)", "(sin ventanitas)"),
    ("Extra 2026-09-21: ", ""),
]
...
_SKEL_COPY_TRANSFORMS = {
    ...
    "workflows/prospectar.md": [
        ("Ma" + "nu", "<TU-NOMBRE>"),
        (
            "Freeze solo con OK por fila.",
            "Freeze solo con OK por fila.\n\nSin token (fallback sin-token): sin "
            "`META_IG_TOKEN_FILE` ni `--token-file` no hay lectura API; trabajar "
            "con evidencia ya congelada en `runs/<slug>/`, declarar \"sin dato\" "
            "donde falte, y pedir el OK del operador antes de cualquier lectura "
            "nueva. Jamas inventar metricas.",
        ),
    ],
    "workflows/juzgar.md": [],
    "workflows/sandbox.md": [],
    "scripts/install-beta.ps1": [],
    "scripts/serve-mobile-task.ps1": [
        ("SIAFB-Mobile-Serve", "<TU-TAREA-SERVE>"),
        ("ver log 2026-09-21", "ver log"),
    ],
    "scripts/serve-prebind.ps1": [
        ("SIAFB-Mobile-Serve", "<TU-TAREA-SERVE>"),
    ],
    "scripts/ig_api.py": [
        ("tokens.priv", "<RUTA-FUERA-DEL-REPO>"),
    ],
    "tests/test_metricas.py": [
        ("ok ma" + "nu", "ok operador"),
    ],
    "tests/test_metricas_pond.py": [
        ("ok ma" + "nu", "ok operador"),
    ],
}
...
_SKEL_ISSUE_BUG = """---
name: Bug report
about: Reportar un fallo del esqueleto beta
---
...
Pega la salida de `python agent.py doctor`:
...
No pegues tokens, claves ni cookies (sin-secretos): redacta todo valor sensible
antes de enviar.
"""

_SKEL_MEMORY = """# Memoria

Solo conocimiento reutilizable. Sin datos de negocio hasta definirlos.

## Reglas de memoria (2 temperaturas)

- **Fria**: `memory_log.jsonl` append-only, 1 hecho por linea JSON (`ts`, `tag`, `fact`).
- **Caliente**: este archivo curado, presupuesto ~100 lineas. Se poda en revision.
...
_SKEL_COSTS = """# Costos por corrida (tokens reales desde opencode.db, lectura read-only)
...
_SKEL_EMBEDDED = {
    ...
    "TROUBLESHOOTING.md": _SKEL_TROUBLESHOOTING,
    ".github/ISSUE_TEMPLATE/bug.md": _SKEL_ISSUE_BUG,
    "memory.md": _SKEL_MEMORY,
    "costs.md": _SKEL_COSTS,
...
def build_skeleton(out_dir):
    ...
    # Cleanup final: el esqueleto publicado no lleva telemetria, bytecode ni
    # corridas reales (solo templates de runs/). Determinista: mismo input,
    # mismos bytes (este bloque corre tambien en el fresh del doctor).
    ...
    for pyc in sorted(out.rglob("*.pyc")):
        try:
            pyc.unlink()
        except OSError:
            pass
    ...
    if runs.exists():
        for child in sorted(runs.iterdir()):
            if child.name in ("brief.template.md", "closeout.template.md"):
                continue
            try:
