...
Sin token (fallback sin-token): sin `META_IG_TOKEN_FILE` ni `--token-file` no hay lectura API; trabajar con evidencia ya congelada en `runs/<slug>/`, declarar "sin dato" donde falte, y pedir el OK del operador antes de cualquier lectura nueva. Jamas inventar metricas.
